...
|    |   |   |   |   |- plugin.min.js
*/
function base_access( $url ) {
	$parsedUrl = parse_url($url);
	($parsedUrl['host'] == 'localhost') ? $url = $parsedUrl['scheme'].'://'.$parsedUrl['host'].'/'.explode('/', $parsedUrl['path'])[1].'/public' : $url = $parsedUrl['scheme'].'://'.$parsedUrl['host'].'/public';
	return $url;
}

// $base_url = base_access($_SERVER['HTTP_REFERER']);
$config = array(
